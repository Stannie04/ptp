...
def add_offset(alignments, offset, author, piece):
    """Add an offset to all alignments."""

    offset_alignments = [('start','end')]
    new_alignments = [(round(float(a[0]) + float(offset), 3), round(float(a[1]) + float(offset), 3)) for a in alignments]

    for a in new_alignments:
        offset_alignments.append(a)

    with open(f"{ALIGNMENT_DIR}/{author}/{author}_{piece}.csv", 'w') as file:
        for a in offset_alignments:
            file.write(f"{a[0]},{a[1]}\n")
        file.close()
